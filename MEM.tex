...
instrument. However, there is another important factor to consider: most
genomes contain repeated sequences, \textit{i.e.} relatively large
subsequences that are present at more than one location. So we cannot map
...
has received substantial attention in bioinformatics. The distinction is
relevant for the present discussion because the candidate solution of the
\emph{true} location problem can only be the \emph{best} location. So in
order to solve the true location problem, we must first solve the best
location problem.
...
best location, $ii.$ the DNA fragment is mapped to another location,
...
candidate locations and reduce the input size of the exact but slow
...
is either a false positive or a false negative.
...
applies to the true location problem. Actually, the process is exactly the
...
target (the true location) is in the candidate set, and that it has the
best score. For now, we will assume that the true location is always the
best and we defer the discussion about this assumption until
section~\ref{sec:not_the_best}.
...
the genome. This will simplify the discussion and we defer the discussion
of this assumption until section~\ref{sec:short_MEMs}
...
To avoid spending too much time on small random MEM seeds, we can
prioritize the seeds so that the best hit is likely to be discovered first
and bail out from the alignments as soon as it appears that their score
will be worse than the best hit so far. We can even put an upper bound on
the quality of candidate so that the alignment can be skipped altogether.
For instance, if we already have a candidate at distance 1 from the read,
there is no need to verify candidates from MEM seeds that do not reach one
end of the read. Indeed, the hit must be at distance at least 2 because
there is a mismatch before the MEM seed and another one after it.
...
choose $\beta=1/2$ so that the term $(1-/\beta)/\beta$ disappears to
simplify expression (\ref{eq:bayes}). We compute this expression for every
read and add the result to the estimated probability of false positive for
the read. Either the term is negligible and it does not affect the
estimated probability, or it is strongly dominant and yields a very strong
probability that the location is incorrect, regardless of the estimate
from the MEM seeding process.
