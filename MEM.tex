...
heuristics to gain speed at a reasonable cost for accuracy. MEM seeding
(where MEM stands for Maximal Exact Match) is a popular heuristic that
shows good empirical performance but for which theory is lacking. It is
thus difficult to calibrate or optimize mapping strategies based on MEM
seeding. Here we develop a theory to estimate the probability that the MEM
seeding heuristic fails to return the correct candidate. The major
innovation is to use concepts from analytic combinatorics to represent
reads as abstract sequences, from which we estimate the probabilities of
interest. We provide two separate methods, which combined together give a
practicial solution to the problem of calibrating MEM seeding when
aligning short reads. These results have implications for MEM-based
heuristics and lay the foundation of a general strategy to tackle sequence
alignment problems.
\end{abstract}
...
reason has received substantial attention in bioinformatics. It turns out
that finding the best location of the read is the only way to get to its
true location, so for now we need to focus our attention on the best
location problem.
