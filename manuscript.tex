...
TODO: mention that this is MEM seeds only. What happens for other seed
types by the way?

Early in the development of the theory, we made the assumption that the
true location is alo the best. This hypothesis was required to claim that
a read is mapped to the true location whenever seeding is on target.
Indeed, if the target is in the candidate set, the exact nature of the
alignment algorithms ensures that the read will be mapped correctly. This
assumption is thus critical because it says that all mapping errors are
caused by off-target seeding.

To know how reasonable this statement is, we have to evaluate how often
the read is mapped incorrectly when seeding is on target. More accurately,
we have to consider the probability that the candidate set contains the
target plus a candidate that is more similar to the read than the target.

The key insight is to realize that MEM seeds tend to exclude the target
from the candidate set if it is not the best location. To develop an
intuition as to why this is the case, consider a read with one sequencing
error. If this is not the best location, then there exists a candidate
location that is a perfect match for the read. But in that case, the read
is covered by a single MEM seed and the target is masked so it cannot be
in the candidate set.

Using simulations, we can show that off-target seeding is much more
frequent than mapping errors after on-target seeding (Table~XXX). In other
words, for the vast majority of mapping errors, the target is not seeded.
This means that, the initial assumption that the true location is always
the best, although incorrect, does not appreciably modify the accuracy of
the estimates for MEM seeds.

For exact and skip-seeds, the conclusions are different.
...
this size when $p=0.01$ (according to Figure~\ref{fig:curves_mem}, the worst
