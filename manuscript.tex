...
contain $s$ mismatches (against any of the two sequences). We thus define
$\tilde{p}$ as the upper bound on the probability of a mismatch,
...
segments is $1 + (qz) + \ldots + (qz)^{\gamma-j-1}$.
...
non-skipped positions because both sequences have match length 0. If the
mismatch against $+$ is in another interval, the terminator is
$\downarrow_{/j}^+$ because the $-$ sequence has a positive match length.

If the next terminator is $\downarrow_{/j}^+$ ($j \geq 1$), then it cannot
be in the same interval as the preceding $\downarrow_{/i}^-$ terminator,
which imposes a lower bound on the size of the segment. Since at the
$\downarrow_{/i}^-$ terminator the match length for $+$ was $i$, there
must be a non-skipped position $i$ nucleotides before. The number of
nucleotides from the $\downarrow_{/i}^-$ terminator to the next
non-skipped position is thus $y = -i \modulo{n+1}$, so the minimum segment
length is $y+1$ There is also an upper bound on the size of the segment
because the match length for the $+$ sequence cannot become higher than
$\gamma-1$, imposing the length to be lower than $\gamma-i$. The shortest
segment is terminated by $\downarrow_{/1}^+$, and the longest by
...
have to distinguish two cases. The first is that the segment length is $y$
nucleotides or shorter, where we have seen that a mismatch against $+$ is
sufficient to produce the $\Downarrow_j$ terminator. The second case is
that the segment length is above $y$ nucleotides, where a double mismatch
is necessary to produce the the $\Downarrow_j$ terminator. We have seen
above that the $\downarrow_{/i}^-$ terminator is located $y$ nucleotides
before the next non-skipped position, so in the first case, only the
terminators from $\Downarrow_0$ to $\Downarrow_{y-1}$ are possible (this
case is impossible if $y=0$). More specifically, short segment terminated
by $\Downarrow_j$ have $y-j-1$ matches followed by a mismatch for the $+$
sequence, with weighted generating function $(az)^{y-j-1}bz$.
