...
identified by their terminator, indicated in the margins for clarity.
...
equivalent to defining $x$ as the number of $\square$ symbols of the
shortest $\Downarrow_j$-segment, with $x = i-j-1 \modulo{n+1}$ and $\ell_0
= x+1$.

The shortest $\Downarrow_j$-segment consists of $x$ symbols $\square$
followed by the $\Downarrow_j$ terminator, so it has weighted generating
function $(qz)^xpz$. The other segments have a multiple of $n+1$
additional $\square$ symbols so their weighted generating functions are
$(qz)^{x+(n+1)}pz, (qz)^{x+2(n+1)}pz, \ldots, (qz)^{x+m(n+1)}pz$. Summing
over those cases, we finally obtain
\begin{eqnarray}
...
Denote $J_i(z)$ the weighted generating function of tails that can follow
a $\Downarrow_i$-segment. There are $\gamma+i$ of them, each consisting of
0 to $\gamma+i-1$ symbols $\square$ followed by the special $|$
...
($i \geq 1$) signify that the nucleotide is a mismatch against the $-$
sequence only, the symbols $\downarrow_{/i}^+$ ($i \geq 1$) signify that
...
as sequences of segments, except that now the terminators are the symbols
$\downarrow_{/i}^-$, $\downarrow_{/i}^+$ and $\Downarrow$ ($i \geq 1$).
The tail, as usual, is terminated by the symbol $|$.

The index $i$ in the symbol $\downarrow_{/i}^-$ indicates the match
length of the $+$ sequence. Likewise, the index $i$ in the symbol
...
sequence and $d$ is the probability that it matches neither. Obviously
$a+b+c+d=1$. These definitions allow us to specify the weighted generating
function of the symbols and of the segments. To specify the transfer
matrix, we only have to make sure that we eliminate the combinations that
create a match of size $\gamma$ or more for any of the two sequences. For
...
exact seed) and $S_1$ is the event that it has a \emph{skip} seed for the
first duplicate (instead of an exact seed).
...
$\downarrow^-_{/i}$ and $\downarrow^+_{/i}$ ($i \geq 1$) have the same
meaning as in the dual alphabet of section~\ref{sec:dual}. The symbols
$\Downarrow_i$ ($0 \leq i \leq n$) indicate that both sequences have match
length 0 and that the next non-skipped position is $i$ nucleotides
downstream.
